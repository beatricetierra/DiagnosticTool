...
def BarGraph(df): 
    ...
    if NeedsBrokenAxis(sum_per_row):
        fig = BarGraphBrokenAxis(sum_per_row, size)
    else:
        fig = BarGraphSimple(sum_per_row, size)
    
    plt.tight_layout()
    #plt.show()
    return(fig)

def BarGraphBrokenAxis(series, size):
    try:
        fig, (ax1,ax2) = plt.subplots(1,2,sharey=True, figsize=size) 
        
        # Find limits of split graph
        left_side_limits, right_side_limits = FindRanges(series)
        ...
        series.plot(ax=ax1,kind='barh')
        series.plot(ax=ax2,kind='barh')
        
        LabelValues(ax1)
        LabelValues(ax2, series)
        ...
        fig = BarGraphSimple(series, size)
    return(fig)

def BarGraphSimple(series, size):
...
def FindRanges(series):
    ...
    right_min_pad = int(0.5*increment2)
...
def LabelValues(ax, series=None):
    # Set limit to avoid annotating left side bars on right side plot
